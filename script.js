...
function displayProductDetail() {
    ...
    function updateProductDisplay(colorData) {
        ...
        const allThumbnails = [colorData.mainImage].concat(colorData.thumbnails.slice(0, 3)); // Fixed typo here
...
function displayCart() {
    ...
    cart.forEach((item, index) => {
        const itemTotal = parseFloat(item.price.replace("SEK", "") * item.quantity);
        subtotal += itemTotal;

        const cartItem = document.createElement("div");
        cartItem.classList.add("cart-item");
        cartItem.innerHTML = `<div class="product">
                        <img src="${item.image}">
                    ...
                    <button class="remove"><i class="ri-close-line" data-index="${index}></i></button>`;

        cartItemsContainer.appendChild(cartItem);
    });

    subtotalEl.textContent = `${subtotal.toFixed(2)} SEK`;
    grandTotalEl.textContent = `${subtotal.toFixed(2)} SEK`;
